/*
    ...
    PATTERN RECOGNITION:
    ----------------------------------------------------
    - Array is sorted + strictly increasing -> binary search is a strong signal.
    - We don't need to find a missing number directly. Instead, we binary
      search on the COUNT of missing numbers before each index (Binary Search
      on Answer pattern).
    - At any index i, the number of positive integers missing BEFORE arr[i]
      is: arr[i] - (i + 1)
        -> because if nothing were missing, arr[i] would equal (i+1).
    ...
    - Loop ends when l > h. At this point, h is the last index where
      missing < k (i.e., the last "safe" index before crossing k).
